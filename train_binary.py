"""
Training script for Binary Classifier (Real vs Fake)
Trains a model to distinguish between real and AI-generated images
"""
...
class BinaryTrainer:
    """Trainer class for binary classification model"""
    
    def __init__(self, model, device, learning_rate=1e-4, model_save_path='binary_model.pth'):
        """
        Initialize trainer
    ...
    def save_history(self, save_path='training_history_binary.json'):
...
def main():
    ...
    if not (real_path.exists() or (dataset_path / 'real_images').exists()):
        print(f"Error: No 'Real/' or 'real_images/' folder found in {dataset_path}")
        print("Please reorganize your dataset to include:")
        print("  - Real vs Fake(AI) Image Dataset/Real/  (contains real images)")
        print("  - Real vs Fake(AI) Image Dataset/Fake/  (contains fake image folders)")
        return
    ...
    print_model_summary(model, num_classes=2)
